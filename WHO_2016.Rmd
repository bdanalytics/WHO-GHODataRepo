...
title: "WHO: LifeExpectancy regression: WHO_2016"
author: "bdanalytics"
...
Data: 
Source: 
    Training:   https://d37djvu3ytnwxt.cloudfront.net/asset-v1:MITx+15.071x_3+1T2016+type@asset+block/WHO.csv
    New:        <newdt_url>  
Time period: 

```{r set_global_options_wd, echo=FALSE}
setwd("~/Documents/Work/Courses/MIT/Analytics_Edge_15_071x/Lectures/LCTR1_WHO")
...
dsp_numeric_feats_dstrb <- function(feats_vctr) {
    ...
    for (batch in 1:ceiling(length(feats_vctr) / batch_size)) {
        ...
        for (feat in feats_vctr[((batch-1)*batch_size + 1) : 
            ...
            gp <- gp + labs(title = feat)
            
            savgp <- gp
            print(gp, vp = viewport(layout.pos.row = ceiling(pltIx / 2.0), 
                                    layout.pos.col = ((pltIx - 1) %% 2) + 1)) 
#             gp <- savgp
#             png(sprintf("%snumeric_feats_dstrb%d.png", glbOut$pfx, batch))
#             print(gp, vp = viewport(layout.pos.row = ceiling(pltIx / 2.0), 
#                                     layout.pos.col = ((pltIx - 1) %% 2) + 1)) 
#             dev.off()
...
if (glb_impute_na_data && 
    ...
    feedImp <- setdiff(feedImp, c(".rnorm", ".pos", ".pos.y"))
